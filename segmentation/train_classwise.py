"""
Train 3d segmentation model with parallel classwise output layer
Computer binary loss on each layer's output
"""
import os
import sys
import time
...
from segmentation.visualize import make_class_wise_segmentation_image_summary
import segmentation.metrics as metrics


def train_class_wise_segmentation(model, training_data_loader, validation_data, optimizer, criterion, device, config,
                       scheduler=None):
    # log writer
    ...
    if not config['debug_mode']:
        writer = SummaryWriter(os.path.join(config['log_dir'], now_date, config['experiment_name'] + '_' + now_time))
    ...
    finished_epochs, best_score = initialize_model(model, optimizer, config['resume'])
    ...
    while current_epoch <= config['n_epochs']:
        ...
        for i, (images, truths, name) in enumerate(training_data_loader):
            ...
            loss = 0
            for c in range(config["model_setting"]["n_classes"]):
                loss += criterion(outputs[:,c,:,:,:], (truths == (c+1)).float().to(device))
            ...
            if config['debug_mode']:
                image_summary = make_class_wise_segmentation_image_summary(images, truths, outputs.cpu())

        if not config['debug_mode'] :
            # and current_epoch % config['save_ckpts_epoch_period'] == 0:
            image_summary = make_class_wise_segmentation_image_summary(images, truths, outputs.cpu())
            writer.add_image("training", image_summary, global_step=global_step)

        # validation
        if current_epoch % config['valid_epoch_period'] == 0 or config['debug_mode']:
            with torch.no_grad():
                model.eval()
                dice_FC = 0
                dice_TC = 0
                ...
                for j in range(len(validation_data) if best_score > 0.7 else 2):
                    ...
                    for i in range(0, np.ceil(image_tiles.size()[0] / valid_batch_size).astype(int)):
                        ...
                        temp_output = model(temp_input.to(device)).cpu()

                        outputs.append(temp_output)
                        del temp_input

                    predictions = torch.sigmoid(torch.cat(outputs, dim=0)) > 0.5

                    pred_assemble_FC = validation_data.transform.transforms[0].assemble(predictions[:, 0, :, :, :],
                                                                                     if_itk=False)
                    pred_assemble_TC = validation_data.transform.transforms[0].assemble(predictions[:, 1, :, :, :],
                                                                                        if_itk=False)

                    dice_FC += metrics.metricEval('dice', pred_assemble_FC, truths.numpy() == 1,
                                                  num_labels=2)
                    dice_TC += metrics.metricEval('dice', pred_assemble_TC, truths.numpy() == 2,
                                                  num_labels=2)
                dice_FC = dice_FC / (j + 1)
                dice_TC = dice_TC / (j + 1)
            ...
            # image_summary = make_segmentation_image_summary(images, truths, outputs)
...
def main(args):
    ...
    config = dict(
        ...
        data_dir=os.path.join(data_root, "Nifti_rescaled_LEFT"),
        valid_data_dir=os.path.join(data_root, "Nifti_rescaled_LEFT"),
    ...
    training_data = data3d.NiftiDataset(config['training_list_file'], config['data_dir'], mode="training",
                                        preload=args.preload, transform=train_transform)
    ...
    validation_data = data3d.NiftiDataset(config['validation_list_file'], config['valid_data_dir'], mode="training",
                                          preload=args.preload  , transform=valid_transform,
                                          )
    ...
    if config['lr_mode'] == 'multiStep':
        scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[300, config['n_epochs'] - 100], gamma=0.1)
    ...
    # training
    train_class_wise_segmentation(model, training_data_loader, validation_data, optimizer, criterion, device, config,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--preload', action='store_true',
                        help='if preload data into memory')

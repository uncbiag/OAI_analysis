...
def generate_deform_grid(transform, slice_axis, background_image=None):
    """
    Abandoned
    ...
    for i, axis in enumerate(left_axis):
        ...
        ax.contour(T_slice, colors=['yellow'], linewidths=10.0, linestyles='solid', levels=np.linspace(-1, 1, 20))
    canvas = FigureCanvas(fig)
    canvas.draw()
    width, height = fig.get_size_inches() * fig.get_dpi()
    image = np.fromstring(canvas.tostring_rgb(), dtype='uint8').reshape(int(height), int(width), 3)/255
...
def test_generate_deform_grid():
    ...
    transform3d = get_identity_transform_batch([1, 1, 160, 160, 160])
    idslice = transform3d[0,:, 80, :, :]
    bk_img = torch.ones(1,160,160)/2
...
def make_segmentation_image_summary(images, truths, raw_output, maxoutput=4, overlap=True):
    ...
    for n in range(n_samples):
        for axis in range(3):
            ...
            if (source_seg is not None) and (target_seg is not None) and (warped_source_seg is not None):
                source_seg_slice = torch.select(source_seg[n, 0, :, :, :], axis - 1, slice_ind)
                source_seg_slice = labels2colors(source_seg_slice, images=source_image_slice.squeeze(0), overlap=True)

                target_seg_slice = torch.select(target_seg[n, 0, :, :, :], axis - 1, slice_ind)
                target_seg_slice = labels2colors(target_seg_slice, images=target_image_slice.squeeze(0), overlap=True)

                warped_source_seg_slice = torch.select(warped_source_seg[n, 0, :, :, :], axis - 1, slice_ind)
        ...
        grids['images'] = vision_utils.make_grid(image_slices, pad_value=1, nrow=3, normalize=True, range=(0, 1))
        if seg_slices:
            grids['masks'] = vision_utils.make_grid(seg_slices, pad_value=1, nrow=3)
        grids['disp_field'] = vision_utils.make_grid(disp_slices, pad_value=1, nrow=3, normalize=True, range=(-0.3, 0.3))
    ...
    if len(labels.size()) == 3:
        ...
        if overlap:
            if images is None:
                ...
                labelcolor = color.label2rgb(labels.cpu().numpy(), images.cpu().numpy(), bg_label=0, alpha=0.7)
        else:

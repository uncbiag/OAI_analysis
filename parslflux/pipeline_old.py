    ...
    def get_current_throughput(self, rmanager, phase_index):
        ...
        for resource_id in current_executors:
            ...
            exectime = resource.get_exectime(self.name,
                                             self.resourcetype)  # TODO: get latest info, not long executimes history
            if exectime == 0:
            ...
            while resource_index < len(list(sorted_idle_times.keys())) and len(prev_output_times) > 0:
                ...
                if time_left >= time_taken:
                    ...
                    if start_time > resource_data[0]:
                        # split idle times
                        sorted_idle_times[resource_key].insert(-1, [resource_data[0], start_time,
                                                                    start_time - resource_data[0]])
...
class PipelineManager:
            ...
            for pipelinestage in self.pipelinestages:
                if pipelinestage.resourcetype == resourcetype:
                    ...
                    if exectime == 0:
                        ...
                        throughput = 1 / exectime
                    performance_to_cost_ratio += throughput / resource.get_cost(resourcetype) * pipelinestage_weights[
                        pipelinestage.name]
    ...
    def scale_down_configuration(self, rmanager, pipelinestageindex, overallocation, throughput, available_resources):
        ...
        while True:
            weighted_pcr_ranking = self.get_weighted_performance_to_cost_ratio_ranking(rmanager,
    ...
    def scale_down_configuration_imbalance_limit(self, rmanager, pipelinestageindex, overallocation, throughput,
                                                 available_resources, imbalance_limit):
        to_be_deleted = []
        target_throughput = float((1 - overallocation)) * throughput
        ...
        while True:
            ...
            for resource_id in weighted_pcr_ranking.keys():
                ...
                print('scale_down_configuration', resource_id, resource_name, resource_throughput, throughput,
                      target_throughput_lb, target_throughput_ub)
        ...
        while True:
            ...
            for resource_name in weighted_pcr_ranking.keys():
                ...
                print('scale_up_configuration', resource_name, resource_throughput, resource_available, throughput,
                      added_throughput, target_throughput)
    ...
    def scale_up_configuration_limit_imbalance_limit(self, rmanager, pipelinestageindex, input_pressure, throughput,
                                                     resource_limit, imbalance_limit):
        to_be_added = {}
        target_throughput = input_pressure - throughput
        ...
        while True:
            if throughput >= target_throughput_lb and throughput <= target_throughput_ub:
                print('scale_up_configuration_limit: within range', throughput, target_throughput_lb,
                      target_throughput_ub)
                break
            weighted_pcr_ranking = self.get_weighted_performance_to_cost_ratio_ranking_all(rmanager,
                                                                                           pipelinestage.resourcetype)
    ...
    def scale_up_configuration_limit(self, rmanager, pipelinestageindex, input_pressure, throughput, resource_limit,
        ...
        while True:
            ...
            for resource_name in weighted_pcr_ranking.keys():
                ...
                print('scale_up_configuration_limit', resource_name, resource_throughput, resource_available,
                      throughput, target_throughput, resource_limit)
        ...
        while pipelinestageindex >= 0:
            ...
            if prev_sametype_pipelinestage != None:
                upcoming_throughputs[str(pipelinestageindex)] = pipelinestage.get_free_resource_throughput(rmanager,
                                                                                                           prev_sametype_pipelinestage.phases[
                                                                                                               0].current_executors)
    ...
    def reconfiguration_down(self, rmanager, current_time, free_cpus, free_gpus):
        ...
        while pipelinestageindex < len(self.pipelinestages):
            ...
            if str(pipelinestageindex) in underallocations and underallocations[
                str(pipelinestageindex)] >= 1.0 and total_throughput <= 0:
    ...
    def reconfiguration_up_down_underallocations(self, rmanager, current_time, free_cpus, free_gpus, imbalance_limit,
        ...
        while pipelinestageindex < len(self.pipelinestages):
            ...
            if str(pipelinestageindex) in underallocations and pending_workitems > 0 and (
                    (underallocations[str(pipelinestageindex)] >= 1.0 and total_throughput <= 0) or (
                    underallocations[str(pipelinestageindex)] > 0 and throughputs[
                str(pipelinestageindex)] == total_throughput)):
    ...
    def parse_pipelines(self, rmanager):
        ...
        while index < len(pipelinestage_resourcetypes):
            if current_resourcetype == pipelinestage_resourcetypes[index]:
                ...
                self.pipelinestages.append(
                    PipelineStage(current_index, current_names, current_resourcetype, rmanager, self.batchsize))
                current_index = index
                current_names.clear()
                current_resourcetype = pipelinestage_resourcetypes[index]
                current_names.append(pipelinestage_names[index])
    ...
    def check_workitem_waiting_idleness(self, rmanager, resource_id, resourcetype, last_phase_closed_index,
                                        target_pipelinestage, current_time):
        ...
        for pipelinestage in self.pipelinestages:
            if pipelinestage.resourcetype == resourcetype:
                if pipelinestage_index + 2 < len(self.pipelinestages):
                    ...
                    print('check_workitem_waiting_idleness ()', pipelinestage.name, current_phase_index,
                          next_sametype_pipelinestage.phases[current_phase_index].total_count)
                    if next_sametype_pipelinestage.phases[current_phase_index].total_count == 0:
                        ...
                        print('check_workitem_waiting_idleness ()', next_sametype_pipelinestage.name,
                              current_phase_index)
                        print('check_workitem_waiting_idleness ()',
                              next_pipelinestage.phases[current_phase_index].current_executors)
                        min_time_left = -1
                        ...
                        if min_time_left != -1 and target_exectime > min_time_left:
                            print('check_workitem_waiting_idleness ()', pipelinestage.name, current_phase_index,
                                  min_time_left, resource_id, target_exectime)
                            return False
                        break
                    else:
                        return False
    ...
    def predict_execution_fixed(self, rmanager, current_time, batchsize, last_phase_index_closed,
        ...
        while index < last_phase_index_closed + (no_of_prediction_phases + 1) and index < self.no_of_columns:
            ...
            for pipelinestage in self.pipelinestages:
                ...
                work_to_be_done = queued_work + phase.pcurrent_count - len(phase.pcurrent_executors) + (
                            phase.target - phase.ptotal_count)

                if phase.pending_output == 0:
                    pipelinestage_index += 1
                    continue

                print(phase.pipelinestage, queued_work, phase.current_count, phase.pcurrent_count, phase.total_complete,
                      phase.ptotal_complete, phase.total_count, phase.ptotal_count)
                ...
                if prev_pipelinestage_phase == None:
                    ...
                    if prev_sametype_phase != None:
                        ...
                        if prev_pipelinestage_phase.pfirst_workitem_completion_time > prev_sametype_phase.pfirst_resource_release_time:
                            ...
                            print('idle period 1', prev_sametype_phase.pipelinestage,
                    ...
                    if prev_pipelinestage.all_resource_service_rate > pipelinestage.all_resource_service_rate:
                        finish_time, first_resource_release_time, end_times = pipelinestage.get_time_required_3(
                            rmanager,
                            work_to_be_done,
                            current_time,
                        ...
                        if queued_finish_time >= prev_pipelinestage_phase.pendtime:
                            ...
                            for resource in pipelinestage_resources:
                                if resource.id in phase.pcurrent_executors:
                                    starttimes[
                                        resource.id] = idle_period_start + idle_period  # TODO: use a function get_queued_work_resource() that looks into either get_work_left or pqueued {}
                                    if resource.id in phase.pqueued.keys():
                                        idle_periods[resource.id] = [
                                            [phase.pqueued[resource.id][1][2], idle_period_start + idle_period,
                                             idle_period_start + idle_period - phase.pqueued[resource.id][1][2]]]
                                    else:
                                        time_left = resource.get_time_left(phase.resourcetype, current_time)
                                        idle_periods[resource.id] = [
                                            [current_time + time_left, idle_period_start + idle_period,
                                             idle_period_start + idle_period - (current_time + time_left)]]
                                else:
                                    starttimes[resource.id] = idle_period_start + idle_period
                                    idle_periods[resource.id] = [
                                        [idle_period_start, idle_period_start + idle_period, idle_period]]
                            ...
                            if phase.pstarttime >= current_time:
                                ...
                                phase.pfirst_workitem_completion_time = phase.pstarttime + (
                                            1 / avg_resource_service_rate)
                    ...
                    if pipelinestage_index == len(self.pipelinestages) - 1:
                        ...
                        print('idle period 4', prev_pipelinestage_phase.pipelinestage,
                              [prev_pipelinestage_phase.pendtime, phase.pendtime], idle_periods)
            ...
            self.pre_compute_prediction(rmanager, batchsize, cpu_idle_periods, gpu_idle_periods, index + 1,
                                        prediction_key)
    ...
    def pre_compute_prediction(self, rmanager, batchsize, cpu_idle_periods, gpu_idle_periods, phase_index,
        ...
        while current_phase_index < self.no_of_columns:
            ...
            new_cpu_idle_periods, new_gpu_idle_periods, starttimes_dict, work_done_dict, fractional_work_dict = \
                self.calculate_early_computation(rmanager, batchsize, new_cpu_idle_periods, new_gpu_idle_periods,
                                                 current_phase_index, fractional_work_allowed)
            ...
            for pipelinestage_name in work_done_dict.keys():
                ...
                if work_done_dict[pipelinestage_name] > 0.0:
                    ...
                    if current_phase.ptotal_complete > (batchsize - len(pipelinestage_resources)):
                        if current_phase.pfirst_resource_release_time == -1:
                            current_phase.pfirst_resource_release_time = current_phase.persistent_p_outputs[
                                batchsize - len(pipelinestage_resources)]
                ...
                if current_phase.pendtime != -1:
                    current_phase.predictions[prediction_key] = [prediction_key, current_phase_index,
                                                                 current_phase.pipelinestage.split(':')[0],
                                                                 current_phase.starttime, current_phase.pstarttime,
                                                                 current_phase.pendtime,
    ...
    def print_stage_queue_data_2(self, rmanager):
        ...
        for pipelinestage in self.pipelinestages:
            ...
            for phase in pipelinestage.phases:
                plot_data[pipelinestage.name].append(
                    [phase.queue_snapshots, phase.starttime, phase.endtime, phase.predictions])

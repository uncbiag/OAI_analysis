...
class FastCompleteFirstServe7Alloc (Policy):
    def add_new_workitems (self, rmanager, imanager, pmanager, empty_resources, resourcetype):
        ...
        for workitem in workitems:
            ...
            exectimes = {}
            need_relative_ranking = False
            for resource in resources:
                exectime = resource.get_exectime (encoded_workitem_pipelinestages)
                exectimes[resource.id] = exectime
        ...
        for workitem_id in rankings.keys ():
            workitem_rankings = rankings[workitem_id]

            for resource_id in workitem_rankings.keys ():
                if workitem_rankings[resource_id] == 10000:
                    workitem_rankings[resource_id] = 0

            rankings[workitem_id] = dict(sorted(rankings[workitem_id].items(), key=lambda item: item[1]))
        ...
        while True:
            ...
            for workitem_id in best_rankings:
                ...
                resource_id = best_rankings[workitem_id][1]
                ...
                if best_ranking == None:
                    ...
                    if best_ranking[0] == ranking and ranking < (0.5 * len (resources)) and best_ranking[1] == resource_id:
                        continue
                    best_ranking = best_rankings[workitem_id]
                    best_ranking_workitem_id = workitem_id
            ...
            if len (pending_workitems_dict) > 0 and best_ranking_workitem_id in new_workitems_dict.keys ():
                print ('it works')
